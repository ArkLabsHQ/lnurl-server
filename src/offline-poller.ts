...
export async function settleOfflineSwaps(store: SettlementStore, creator: OfflineSwapCreator, recovered?: OfflineSwapStore): Promise<number> {
  let settled = 0;
...
export function startOfflineSettlementPoller(
  ...
  const timer = setInterval(() => {
    ...
    if (inFlight) return;
    inFlight = true;
    void settleOfflineSwaps(store, creator, recovered).finally(() => {
      inFlight = false;
    });
  }, intervalMs);
