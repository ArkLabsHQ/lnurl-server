...
export default defineConfig({
  ...
  webServer: {
    ...
    // The client resolves through its dist/, so it is rebuilt first or the suite tests a stale one.
    command: `pnpm -w run build:client && npx vite build && npx vite preview --port ${WALLET_PORT} --strictPort --host 127.0.0.1`,
