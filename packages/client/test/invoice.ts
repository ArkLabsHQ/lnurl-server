...
/**
 * A structurally valid bolt11 carrying only a description and the payment
 * hash; the signature is zeros. `amountSat`, when given, is encoded in the
 * HRP with the `n` multiplier (value = amountSat * 10, so value * 100 msat
 * always equals amountSat * 1000 msat exactly); omitted, the invoice is
 * amountless.
 */
export function buildInvoice(paymentHashHex: string, amountSat?: number): string {
