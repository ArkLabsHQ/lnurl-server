...
A refused write is not undone: it stays in the enclave's database and lands with the next checkpoint that succeeds, so a client refused this way should re-read before retrying. The design answers that ambiguity with operation identifiers, and restores the committed state before resuming after a failed commit. Neither is built; a restart is what restores it.
