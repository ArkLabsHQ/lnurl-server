...
- **Offline receive (opt-in)** — when `BOLTZ_URL` + `COVCLAIMD_URL` + `ARK_NETWORK` are set, an LN address can register an Arkade receive identity via `POST /lnurl/address/:username/arkade` and receive while its wallet is offline: the server creates a non-interactive Boltz reverse swap paying the user's Arkade address, covclaimd claims the VHTLC (covenant-constrained to the user, so it can't redirect funds), and LUD-21 `verify` reports settlement by polling Boltz. The server holds no user keys or funds. Adds `settlements.swap_id` (migration 004) and per-address `arkade_address` / `claim_public_key` (migration 005). Depends on the `@arkade-os/boltz-swap` non-interactive helper (arkade-os/ts-sdk#625, pending publish); the on-chain path is unverified in CI.
