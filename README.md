...
Normally an LN address only resolves while the wallet's SSE session is connected. With offline receive, a wallet can receive while disconnected: the server quotes a **solver-mediated swap over the Arkade intents corridor** (`lightning:BTC -> arkade:BTC`) and hands the payer the solver's hold invoice. When the payer pays, the solver funds a VHTLC pinned to the user's Arkade address, and **this server claims it** on the user's behalf — constrained by covenant (`enforcePayTo`) to pay only that address, so neither the solver nor the claimer can redirect funds. See [Self-claim](#self-claim-offline_self_claim), which is the intended configuration; a **covclaimd** daemon can claim the same leaf alongside it.
...
> The flag still defaults to `false` so that enabling it stays an explicit act. That default is load-bearing beyond this rail: `offlineReceive.enabled` is derived as `arkServerUrl && (covclaimdUrl || (selfClaim && emulatorUrl))`, so flipping it would also switch the offline-swap rail on for a deployment that configured only the covenant rail.
...
| `OFFLINE_SELF_CLAIM` | `false` | **Set this.** `true` pushes each lockup's `nonInteractiveClaim` leaf here (alongside covclaimd when it is configured), which is how the rail is meant to run — left off, covclaimd is the single point of failure and cannot claim this covenant today. Needs no key — the leaf is signed by the operator and the emulator, and gated on the preimage this server already holds. Requires `OFFLINE_EMULATOR_URL`; `COVCLAIMD_URL` may be omitted. See [Self-claim](#self-claim-offline_self_claim). |
