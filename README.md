...
| `OFFLINE_SELF_CLAIM` | `false` | `true` pushes each lockup's `nonInteractiveClaim` leaf here (and as well as covclaimd when it is configured), so covclaimd stops being a single point of failure. Needs no key — the leaf is signed by the operator and the emulator, and gated on the preimage this server already holds. Requires `OFFLINE_EMULATOR_URL`; `COVCLAIMD_URL` may be omitted. See [Self-claim](#self-claim-offline_self_claim-default-off). |
